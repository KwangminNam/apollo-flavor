...
describe('SuspenseQuery', () => {
  ...
  describe('기본 렌더링', () => {
    ...
    it('변수 없이도 동작해야 한다', async () => {
      ...
      await act(async () => {
        render(
          <MockedProvider mocks={mocks} addTypename={false}>
            <Suspense fallback={<div data-testid="loading-users">Loading users...</div>}>
  ...
  describe('Apollo Client 옵션', () => {
    it('fetchPolicy 옵션을 지원해야 한다', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByTestId('user-with-fetch-policy')).toHaveTextContent('John Doe');
      });
    });
    ...
    it('errorPolicy 옵션을 지원해야 한다', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByTestId('user-with-error-policy')).toHaveTextContent('John Doe');
      });
    });
  });
